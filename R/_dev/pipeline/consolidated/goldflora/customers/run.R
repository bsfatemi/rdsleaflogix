...
blaze_customers <- dbReadTable(pg, "goldflora_blaze_customers")
dbd(pg)
treez_customers <- build_trz2_customers(treez_customers, org) |>
  mutate(customer_id = paste0(store, "-", customer_id))

# Join --------------------------------------------------------------------
# Discard Blaze phone numbers that are repeated in multiple Treez customers.
...
customers <- left_join(
  treez_customers, select(blaze_customers, blaze_customer_id, phone),
  by = "phone"
)
# Bind Blaze customers that couldn't be joined by phone number.
customers <- bind_rows(customers, anti_join(blaze_customers, customers, by = "phone"))
