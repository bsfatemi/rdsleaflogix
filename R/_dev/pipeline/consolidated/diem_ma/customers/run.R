...
box::use(
  ...
  dplyr[
    arrange, bind_rows, coalesce, count, desc, distinct, filter, group_by, left_join, mutate, pull,
    ungroup, row_number, select
  ],
...
flowhub_customers <- dbReadTable(pg, "diem_ma_flowhub_customers")
dbd(pg)
...
# Discard FlowHub phone numbers that are repeated in multiple LL customers.
...
flowhub_optins <- arrange(flowhub_customers, desc(as_datetime(last_updated_utc))) |>
...
# For each joined customer in LL, give their FlowHub `customer_id`.
customers <- left_join(
  ll_customers, select(flowhub_customers, flowhub_customer_id, phone),
  by = "phone"
) |>
  left_join(flowhub_optins, by = "phone") |>
  mutate(pos_is_subscribed = coalesce(pos_is_subscribed, FALSE))
# Bind FlowHub customers that couldn't be joined by phone number.
customers <- bind_rows(
  customers,
  filter(flowhub_customers, !phone %in% customers$phone)
)
