...
akerna_customers <- dbReadTable(pg, "lionheart_akerna_customers")
dbd(pg)
...
# Discard akerna phone numbers that are repeated in multiple LL customers.
...
akerna_optins <- arrange(akerna_customers, desc(as_datetime(last_updated_utc))) |>
...
# For each joined customer in LL, give their akerna `customer_id`.
customers <- left_join(
  ll_customers, select(akerna_customers, akerna_customer_id, phone),
  by = "phone"
) |>
  left_join(akerna_optins, by = "phone") |>
  mutate(pos_is_subscribed = coalesce(pos_is_subscribed, FALSE))
# Bind akerna customers that couldn't be joined by phone number.
customers <- bind_rows(
  customers,
  filter(akerna_customers, !phone %in% customers$phone)
)
