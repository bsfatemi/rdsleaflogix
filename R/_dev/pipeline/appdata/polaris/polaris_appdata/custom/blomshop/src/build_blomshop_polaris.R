...
build_blomshop_polaris <- function(customers, customer_behavior, abandoned_carts,
                                   blomshop_mapping) {
  ...
  output <- customers |>
    ...
    mutate(
      last_order_facility = if_else(customer_id %in% blomshop_mapping$customer_id,
        "The Blóm Shop", coalesce(last_order_facility, "Never Ordered")
      ),
