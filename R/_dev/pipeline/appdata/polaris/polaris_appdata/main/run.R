...
# SOURCE ------------------------------------------------------------------
source(
  "inst/pipeline/appdata/polaris/polaris_appdata/main/src/build_polaris_appdata.R",
  local = TRUE
)
...
orgs <- plAppDataIndex()[
  polaris_is_active & current_client & !polaris_manual_pl, unique(short_name)
]
orgQuery <- paste(orgs, collapse = "', '")
...
optin_status <- dbReadTable(cn, "optin_status")
dbd(cn)

#  POLARIS-APPDATA ---------------------------------------------------------------
polaris <- build_polaris_appdata(customers, customer_behavior, optin_status, abandoned_carts)
...
dbCommit(pg)
dbExecute(pg, "VACUUM ANALYZE polaris_appdata")
dbd(pg)
