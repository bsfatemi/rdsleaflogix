# source ----------------------------------------------------------------------
source("inst/pipeline/cabbage_patch/meadow/inventory_snapshot/init.R", local = TRUE)
...
if (length(errored) > 0 && length(errored) == nrow(curr_clients)) {
  stop("Meadow stock snapshot failed for all orgs (", length(errored), ").")
}
...
dbAppendTable(cn, "meadow_rooms_stock_snapshot", stock_snapshot)
dbExecute(cn, "ANALYZE meadow_rooms_stock_snapshot")
dbd(cn)
